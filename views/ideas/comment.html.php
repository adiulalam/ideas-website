...
function commentMutationCheck($CommentID, $totalComments)
{
    ...
    if (in_array($CommentID, $totalComments)) {
        ...
        <form action='' method='post' class=' float-right inline-flex items-center '>
            ...
            <Button type='button' data-modal-toggle='$CommentID' class=' float-right inline-flex items-center mx-1 py-1 px-3 text-sm font-medium text-center text-white rounded-lg focus:ring-4 focus:outline-none bg-red-600 hover:bg-red-700 focus:ring-red-800'>Delete</Button>
        </form>
        ...
        <div id='$CommentID' tabindex='-1' class='hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 md:inset-0 h-modal md:h-full'>
            <div class='relative p-4 w-full max-w-md h-full md:h-auto'>
                <div class='relative rounded-lg shadow bg-gray-700'>
                    <button type='button' class='absolute top-3 right-2.5 text-gray-400 bg-transparent rounded-lg text-sm p-1.5 ml-auto inline-flex items-center hover:bg-gray-800 hover:text-white' data-modal-toggle='$CommentID'>
                    ...
                    <div class='p-6 text-center'>
                        ...
                        <h3 class='mb-5 text-lg font-normal text-gray-400'>Are you sure you want to delete this Comment?</h3>
                        <form action='' method='post'>
                            <input type='hidden' name='ID' value='$CommentID'>
                            <button data-modal-toggle='$CommentID' type='submit' name='action' value='deleteComment' class='text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2'>
                                Yes, I'm sure
                            </button>
                        <button data-modal-toggle='$CommentID' type='button' class='focus:ring-4 focus:outline-none rounded-lg border text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 bg-gray-700 text-gray-300 border-gray-500 hover:text-white hover:bg-gray-600 focus:ring-gray-600'>No, cancel</button>
                        </form>
                    </div>
                </div>
            </div>
