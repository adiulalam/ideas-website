<div class="sm:flex justify-center py-1">
    <div class="float left mx-5 my-1 ">
        <form method="get" action="?">
            <input type="hidden" name="page" value="<?php echo isset(($_GET['page'])) ? ($_GET['page']) : 1; ?>">
            <select Name="orderBy" ID="orderBy" class="bg-gray-50 border text-center border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 px-6 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" onchange='this.form.submit()'>
    ...
    <div class="float right mx-5 my-1">
        <form method="get" action="?">
            <input type="hidden" name="page" value="<?php echo isset(($_GET['page'])) ? ($_GET['page']) : 1; ?>">
            <input type="hidden" name="orderBy" value="<?php echo isset(($_GET['orderBy'])) ? ($_GET['orderBy']) : 'IdeaDate DESC'; ?>">
            <select name="limitRecords" id="limitRecords" class="bg-gray-50 border text-center border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 px-6 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" onchange='this.form.submit()'>
                <option disabled="disabled" selected="selected">Limit: </option>
                <?php foreach ([10, 20, 50, 100] as $offset) : ?>
                    <option <?php if (isset($_GET["limitRecords"]) && $_GET["limitRecords"] == $offset) echo "selected" ?> value="<?= $offset; ?>"><?= 'Limit: ' . $offset; ?></option>
                <?php endforeach; ?>
            </select>
            <input type="hidden" name="Author" value="<?php echo isset($_GET["Author"]) ? $_GET["Author"] : '' ?>">
            <input type="hidden" name="Category" value="<?php echo isset($_GET["Category"]) ? $_GET["Category"] : '' ?>">
