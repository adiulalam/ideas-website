    ...
    <div class="flex p-5">
        <div class="group inline-block mr-1">
            <select Name="Author" ID="Author" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option selected="selected" value="">Authors:</option>
                <?php foreach ($authors as $Author) : ?>
                    <option <?php if (isset($_GET["Author"]) && $_GET["Author"] == $Author['ID']) echo "selected" ?> value="<?php html($Author['ID']); ?>"><?php html($Author['Name']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="group inline-block mr-1">
            <select Name="Category" ID="Category" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                <?php foreach ($categories as $Category) : ?>
                    <option <?php if (isset($_GET["Category"]) && $_GET["Category"] == $Category['ID']) echo "selected" ?> value="<?php html($Category['ID']); ?>"><?php html($Category['Name']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="relative w-full">
            <input type="search" name="text" id="text" value="<?php echo isset($_GET["text"]) ? $_GET["text"] : '' ?>" class="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-lg border-l-gray-50 border-l-2 border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-l-gray-700  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:border-blue-500" placeholder="Search ideas">
